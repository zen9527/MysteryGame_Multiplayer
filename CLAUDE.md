...
- **Shared**: Pydantic models (server/models.py) and ws_types.py define the contract; frontend has matching Zod schemas
...
shared/          ws_types.py — Pydantic schemas for WebSocket messages
...
- **Frontend State**: Pinia store `useGameStore` manages phase, messages, players, currentEvent. WebSocketManager (`utils/ws.ts`) handles raw WS connections.
...
- Server also reads from `server/.env` (see start.bat)
- Runtime LLM config can be changed via `/api/llm-config` POST without restarting

## Game Flow

1. Admin creates room → `waiting` phase, placeholder script
2. Players join → roles assigned from script (script must be generated first)
...
- The `_env` file at project root is used; `server/.env` is also referenced by start.bat.
